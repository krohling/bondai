...
class GroupConversationConfig(ABC):
    ...
    def get_reachable_members(self, 
        ...
        reachable_members = list(set([m for m in self._members if m != member and m.name.lower() != member_name.lower()]))
...
class TeamConversationConfig(BaseGroupConversationConfig):
    ...
    def get_reachable_members(self, 
        ...
        member_name = '' if not member_name else member_name
        member_teams = [t for t in self._teams for m in t if m == member or m.name.lower() == member_name.lower()]
        reachable_members = list(set([m for t in member_teams for m in t if m != member and m.name.lower() != member_name.lower()]))
...
class CompositeConversationConfig(BaseGroupConversationConfig):
    ...
    def __init__(self, *conversation_configs: List[BaseGroupConversationConfig]):
        self._conversation_configs: List[BaseGroupConversationConfig] = list(conversation_configs)
    ...
    def get_reachable_members(self, 
        ...
        return list(set([m for c in self._conversation_configs for m in c.get_reachable_members(member, member_name)]))