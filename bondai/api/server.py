...
class BondAIAPIServer:
    ...
    def get_agent_by_id(self, agent_id: str) -> ConversationalAgent | None:
        agent_registration = next((r for r in self.agent_registrations if r.conversational_agent.id == agent_id), None)
    ...
    def _handle_client_message(self, message):
        ...
        if message.get('event') == 'user_message':
            agent_registration = next((r for r in self.agent_registrations if r.conversational_agent.id == message['data']['agent_id']), None)
            if agent_registration:
