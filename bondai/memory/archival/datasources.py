...
class PersistentArchivalMemoryDataSource(ArchivalMemoryDataSource):
    ...
    def _rebuild_index(self):
        self._index = faiss.IndexFlatL2(self._embedding_model.embedding_size)
        if self._data:
            embeddings = np.array([d['embedding'] for d in self._data]).astype('float32')
    ...
    def search(self, query: str, page: int = 0) -> List[str]:
        query_embedding = np.array(self._embedding_model.create_embedding(query)).astype('float32')
        _, indices = self._index.search(query_embedding, self._page_size * (page + 1))
        result_indices = indices[0][page * self._page_size:(page + 1) * self._page_size]
...
class InMemoryArchivalMemoryDataSource(ArchivalMemoryDataSource):
    ...
    def insert(self, content: str):
        embedding = np.array(self._embedding_model.create_embedding(content)).astype('float32')
        self._data.append(content)
        self._embeddings.append(embedding)
        self._rebuild_index()

    def insert_bulk(self, content: List[str]):
        content_embeddings = np.array(self._embedding_model.create_embedding(content)).astype('float32')
    ...
    def search(self, query: str, page: int = 0) -> List[str]:
        ...
        query_embedding = np.array(self._embedding_model.create_embedding(query)).astype('float32')
        start_idx = page * self._page_size  # Calculate the starting index for the current page
        end_idx = start_idx + self._page_size  # Calculate the ending index for the current page
